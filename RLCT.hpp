...
private:
    ...
    U R(U); //(作用素)->作用素 作用素から逆元を作る
    T RM(T,T); //(要素,要素)->要素 要素から範囲外の要素を引く
...
public:
    RLCT(vector<vector<ll>> &edge,ll rootnum,T err_T,U err_U):lca(edge.size(),rootnum),Proot(rootnum),err_T(err_T),err_U(err_U),root(NULL){
    ...
    RLCT(vector<vector<ll>> &edge,ll Proot,vector<ll> v,T err_T,U err_U):Proot(Proot),err_T(err_T),err_U(err_U),lca(edge.size(),Proot){
        index.resize(edge.size());
        for(int i=0;i<edge.size();i++){
    ...
    void update(ll s,ll t,U u){
        ...
        I To(index[t]);
        if(s!=m){
            U up=J(u,false,lca.high(s),lca.high(m),lca.high(t));
            Link(S);
            update(S,up,0,lca.high(s));
            splay(M);
            update(M,R(up),0,lca.high(m));
        }
        U down=J(u,true,lca.high(s),lca.high(m),lca.high(t));
        Link(To);
        update(To,down,0,lca.high(t));
        splay(M);
        update(M,R(down),0,lca.high(m)-1);
    }
    ...
    T search(ll s,ll t){
        ...
        if(s!=m){
            Link(S);
            ret=F(ret,sum(S,0,lca.high(s)));
            splay(M);
            ret=RM(ret,sum(M,0,lca.high(m)));
        }
        Link(To);
        ret=F(ret,sum(To,0,lca.high(t)));
        splay(M);
        ret=RM(ret,sum(M,0,lca.high(m)-1));
...
/*
 ...
 T RLCT<T,U>::F(T a,T b){
 return (a+b)%MOD;
 }
 
 //(区間,作用素)->要素 区間に作用素を作用させる [high,low]
 template<typename T,typename U>
 T RLCT<T,U>::G(T a,ll high,ll low,U u){
 u.F%=MOD; u.S%=MOD;
 T avr=u.F+u.S*high;
 avr+=u.F+u.S*low;
 avr*=low-high+1;
 ...
 //(要素,要素)->要素 要素から範囲外の要素を引く
 template<typename T,typename U>
 T RLCT<T,U>::RM(T a,T b){
 return (a-b)%MOD;
 }
