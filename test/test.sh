...
if [[ $# -eq 0 ]] ; then
    if [[ $CI ]] ; then
        ...
        git config --global user.name "tubuann"
        git config --global user.email "s1260165@u-aizu.ac.jp"

        git remote set-url origin https://tubuann:${GITHUB_TOKEN}@github.com/tubuann/library.git
